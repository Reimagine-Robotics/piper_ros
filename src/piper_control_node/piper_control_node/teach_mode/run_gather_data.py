...
_MODE = flags.DEFINE_enum(
    ...
    "Mode to run: 'collect_poses' for interactive pose collection, "
    "'collect_data' for gravity compensation data collection.",
)
...
def collect_poses_interactively(
  ...
  print("3. Type 'q' + ENTER when done collecting poses")
  print("4. The collected poses will be saved to:", _POSES_PATH.value)
  print("=" * 60)
  ...
  try:
    while True:
      ...
      user_input = (
          input("Press ENTER to capture pose, or 'q' to quit: ").strip().lower()
      )
...
def main(_):
  ...
  # TODO(jscholz) Does this matter for MIT mode?
  ...
  if _MODE.value == "collect_poses":
    ...
    # Move the arm joints using MIT mode controller with low gains for manual manipulation
    ...
    with piper_control.MitJointPositionController(
      ...
      all_samples = generate_samples(robot, controller, sample_poses)
